   ...
   */
   public int findMinimum( int column )
   {
    ...
    // Write your code here:
	    ...
	    int test = intArray[0][column];
	    for ( int row = 0; row < intArray.length; row++ )
	    {
	    	
	    	
	    	if(test>intArray[row][column]) 
	    	{
	    		test=intArray[row][column];
	    		
	    	
	           // needed to create visual feedback
	    	}
	    	animate( row, column,test );
	    }
	  
	    
	   
	        return test; // replace this line with your return statement
   }
